...
export function register (server, options, next) {
  server.route({
    ...
    handler: async function (request, reply) {
      try {
        ...
        await auth.assertAdminRole(request.auth.credentials, 'view-admins', request.params.companyId)

        var admins = await m.Admin.findAll({
          include: [
            {
  ...
  server.route({
    ...
    handler: async function (request, reply) {
      try {
        ...
        var admin = await m.Admin.findOne({
          where: {
  ...
  server.route({
    ...
    handler: async function (request, reply) {
      try {
        ...
        await auth.assertAdminRole(request.auth.credentials, 'manage-admins', request.params.companyId)

        // Create the admin...
        var company = await m.TransportCompany.findById(request.params.companyId)
        var [adminInst, created] = await m.Admin.findCreateFind({
          where: {
        ...
        await company.addAdmin(adminInst, {name: request.payload.name, permissions: request.payload.permissions})
  ...
  server.route({
    ...
    async handler (request, reply) {
      try {
        ...
        var adminInst = await m.Admin.findOne({
          where: {id: request.params.id},
          include: [
            {model: m.TransportCompany, where: {id: request.params.companyId}}
          ]
        })
        if (request.payload.permissions !== undefined) { adminInst.transportCompanies[0].adminCompany.set('permissions', request.payload.permissions) }
        if (request.payload.name !== undefined) { adminInst.transportCompanies[0].adminCompany.set('name', request.payload.name) }
  ...
  server.route({
    ...
    async handler (request, reply) {
      try {
        var m = getModels(request)
        var adminInst = await m.Admin.findById(request.auth.credentials.adminId)
  ...
  server.route({
    ...
    async handler (request, reply) {
      try {
        var m = getModels(request)
        var adminInst = await m.Admin.findCreateFind({
  ...
  server.route({
    ...
    async handler (request, reply) {
      try {
        ...
        // for authorization
        assert((request.auth.credentials.scope === 'admin' &&
...
"Sends the telephone verification code to the phone number. Returns 400 if user not found."
    ...
    async handler (request, reply) {
      try {
        ...
        await (await m.TransportCompany.findById(request.params.companyId))
