...
async function fetchRoutingData (stops) {
  ...
  const [{data: busStops}, {data: relatedRequests}, {data: travelTimes}] = await Promise.all([
  ...
  return {busStops, relatedRequests, travelTimes}
}
...
The user must have a telephone number and a credit card saved.
    ...
    async handler (request, reply) {
      ...
      try {
        ...
        const {busStops, relatedRequests, travelTimes} = await fetchRoutingData(request.payload.stops)
        ...
        const newBid = await createCrowdstartRouteAndBid({user, busStops, relatedRequests, travelTimes})
