...
async function fetchRoutingData (stops) {
  ...
  const [{data: busStops}, {data: relatedRequests}, {data: travelTimes}, {data: paths}] = await Promise.all([
    axios.get(`${process.env.ROUTING_SERVER_URL}/bus_stops/${path}`),
    axios.get(`${process.env.ROUTING_SERVER_URL}/path_requests/${path}?` + querystring.stringify({
...
export async function createCrowdstartRouteDetails (
  ...
  const arrivalTimes = travelTimes.map(x => x).reverse()
    .reduce(
      (acc, travelTime) => {
  ...
  const bestMatch = await Promise.all(stops.map((stopIndex, index) => {
    ...
    return m.Stop.findAll(
      ...
      .then(a => a.length ? a[0] : createFunc(m.Stop, {
      ...
      .then(stopInstance => ({stop: stopInstance, arrivalTime: arrivalTimes[index]}))
  }))
    ...
    config: {
      ...
      validate: {
        payload: {
          ...
          arrivalTime: Joi.number().integer().min(0).max(24 * 60 * 60e3),
        }
      },
...
The user must have a telephone number and a credit card saved.
    ...
    async handler (request, reply) {
      ...
      async function createCrowdstartRouteAndBid ({user, busStops, relatedRequests, travelTimes}) {
        return db.transaction(async (transaction) => {
          ...
          return m.Bid.createForUserAndRoute(user, route, DEFAULT_CROWDSTART_PRICE, {transaction})
        })
      }
      ...
      try {
        ...
        const {busStops, relatedRequests, travelTimes, paths} = await fetchRoutingData(request.payload.stops)

        TransactionError.assert(
          relatedRequests.length >= 15,
          `The total number of requests served by this crowdstart (${relatedRequests.length}) does not meet the minimum number required (15).`
        )

        const newBid = await createCrowdstartRouteAndBid({user, paths, busStops, relatedRequests, travelTimes})

        reply({
          bid: newBid.toJSON(),
          route: (await m.Route.findById(newBid.routeId, {include: [{model: m.Trip, include: [{model: m.TripStop, include: [m.Stop]}]}]}))
            .toJSON()
        })
      } catch (err) {
...
Previews the crowdstart route that will be created with the given options
    ...
    async handler (request, reply) {
      try {
        const {busStops, relatedRequests, travelTimes, paths} = await fetchRoutingData(request.query.stops)
