...
export const register = function register(server, options, next) {
    ...
    try {
      if (event.promotionId) {
        ...
        if (promoInst.params.usageLimit.globalLimit) {
          await m.PromoUsage.addGlobalPromoUsage(
            promoInst.id,
  ...
  server.route({
    ...
    config: {
      tags: ["api"],
      description: `Given a 6-digit alphanumeric referral code,
...
or 404 if not found`,
    ...
    handler: handleRequestWith(
      ...
      refCodeInst =>
        refCodeInst && refCodeInst.params.ownerId
          ? m.User.findById(refCodeInst.params.ownerId)
          : undefined,
      userInst =>
        userInst
