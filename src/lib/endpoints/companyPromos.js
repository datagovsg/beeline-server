...
import {discountingFunctions, refundingFunctions} from "../promotions/functions/discountRefunds"
import {handleRequestWith, authorizeByRole, deleteInst} from "../util/endpoints"
...
export function register (server, options, next) {
  ...
  const withUsageCounts = async (m, db, promos) => {
    ...
    return promos
      .map(p => _.assign({}, p, { counts: _.omit(usageCounts[p.id], 'promoId') }))
  }
  ...
  server.route({
    ...
    handler: handleRequestWith(
      authorize,
      request => m.Promotion.findAll({ where: { 'params.companyId': request.params.id } }),
  ...
  server.route({
    ...
    handler: handleRequestWith(
      authorize,
      findByPromoId,
      async promoCodeInst => promoCodeInst
  ...
  const assertLimitByCompany = (qualifyingCriteria, id) => {
    InvalidArgumentError.assert(
      _.some(qualifyingCriteria, {type: 'limitByCompany', params: {companyId: id}}),
  ...
  const validateQualifyingCriteria = (promoType, qualifyingCriteria) => {
    ...
    InvalidArgumentError.assert(qualifiersByType, `Unrecognised Promo Type ${promoType}`)
      ...
      validate: {
        ...
        payload: {
          ...
          params: Joi.object().keys({
            ...
            creditAmt: Joi.number().min(0).precision(2),

            qualifyingCriteria: Joi.array().items(Joi.object().keys({
              type: Joi.string().required(),
              params: Joi.object().required(),
            })),

            discountFunction: Joi.object().keys({
              type: Joi.string().required(),
              params: Joi.object().required(),
            }).allow(null),
            refundFunction: Joi.object().keys({
              type: Joi.string().required(),
              params: Joi.object().required(),
            }).allow(null),
      ...
      async (promoCodeInst, request) => {
        ...
        if (request.payload.params) {
          if (request.payload.params.qualifyingCriteria !== undefined) {
            ...
            validateQualifyingCriteria(request.payload.type, request.payload.params.qualifyingCriteria)
          }
          ...
          const newParams = _.assign({}, promoCodeInst.params, request.payload.params)
