...
export const register = function register(server, options, next) {
  ...
  server.route({
    ...
    config: {
      ...
      validate: {
        query: Joi.object({
          startTime: Joi.date().default(
            () => common.midnightToday(),
            "0000hrs today"
          ),
    ...
    handler: async function(request, reply) {
      ...
      let ticketQuery = {
        ...
        order: [[{ model: m.TripStop, as: "boardStop" }, "time", "ASC"]],
      }

      if (request.query.transportCompanyId) {
        ticketQuery.include[0].include[1].include[0].where.transportCompanyId =
      ...
      try {
        ...
        reply(tickets.map(addTripCodeToTicket).map(tick => tick.toJSON()))
      } catch (err) {
  ...
  server.route({
    ...
    handler: handleRequestWith(
      (ignored, request, { models }) =>
        models.Ticket.findById(request.params.id, {
          include: [
            {
              model: models.TripStop,
              as: "alightStop",
              ...
              include: [
                {
                  ...
                  attributes: ["routeId"],
                  include: [
                    { model: models.Route, attributes: ["transportCompanyId"] },
                  ],
                },
              ],
            },
      ...
      authorizeByRole(
        "issue-tickets",
        ticket => ticket.alightStop.trip.route.transportCompanyId
      ),
      assertThat(
        ticket => ["void", "valid"].includes(ticket.status),
        InvalidArgumentError,
        "Ticket status has to be valid or void"
      ),
