...
export function register (server, options, next) {
  server.route({
    ...
    async handler (request, reply) {
      try {
        ...
        await auth.assertAdminRole(request.auth.credentials, 'view-drivers', request.payload.transportCompanyId)

        var driverInstance = await m.Driver.find({
  ...
  server.route({
    ...
    async handler (request, reply) {
      try {
        ...
        await auth.assertAdminRole(request.auth.credentials, 'view-drivers', request.params.companyId)

        var drivers = await m.Driver.findAll({
          include: [
            {
              model: m.TransportCompany,
              where: {id: request.params.companyId},
  ...
  server.route({
    ...
    config: {
      ...
      description: 'Allow admin to search/query drivers by telephone/ids. To facilitate adminview',
    },
    handler: async function (request, reply) {
      try {
        var m = getModels(request)
        var companyIds = await auth.getCompaniesByRole(request.auth.credentials, 'view-drivers')
  ...
  server.route({
    ...
    handler: async function (request, reply) {
      try {
        ...
        await auth.assertAdminRole(request.auth.credentials, 'manage-drivers', request.payload.transportCompanyId)

        // add the company to the driver
        var [driverInstance] = await m.Driver.findCreateFind({
          where: {
  ...
  server.route({
    ...
    config: {
      ...
      validate: {
        payload: Joi.object({
          name: Joi.string(),
          telephone: Joi.string().telephone(),
          remarks: Joi.string().default('').allow(''),
        }),
        params: {
          companyId: Joi.number().integer()
        }
    ...
    handler: async function (request, reply) {
      try {
        ...
        await auth.assertAdminRole(request.auth.credentials, 'manage-drivers', request.params.companyId)
    ...
    handler: async function (request, reply) {
      try {
        ...
        if (request.auth.credentials.scope === 'driver') {
          assert.strictEqual(request.params.id, request.auth.credentials.driverId)
        }
        ...
        var driverInstance = await m.Driver.findById(request.params.id, {
  ...
  server.route({
    ...
    handler: async function (request, reply) {
      try {
        ...
        var driverInstance = await m.Driver.findOne({
        ...
        var updatedInfo = {name: request.payload.name}
        if (request.payload.remarks) {
          updatedInfo.remarks = request.payload.remarks
        }

        await driverInstance.transportCompanies[0].driverCompany.update(updatedInfo)
  ...
  server.route({
    ...
    handler: async function (request, reply) {
      try {
        ...
        var driverInst = await m.Driver.findById(request.params.driverId)
  ...
  **/
  server.route({
    ...
    async handler (request, reply) {
      try {
        ...
        var driverInst = await m.Driver.find({
        ...
        var randomCode = leftPad(Math.floor(Math.random() * 1000000), 6, 0)
  ...
  server.route({
    ...
    async handler (request, reply) {
      try {
        var m = getModels(request)
        var driverInst = await m.Driver.findById(request.auth.credentials.driverId)
