...
import { getNextDayInWeek } from "../util/common"
import axios from "axios"
import * as polyline from "polyline"

const DEFAULT_CROWDSTART_PRICE = 5.0
const DEFAULT_CROWDSTART_VALIDITY = moment.duration(3, "months") // 3 months from now
const DEFAULT_CROWDSTART_START = moment.duration({ months: 3, weeks: 2 }) // 3 months and 2 weeks from now
...
function getSchedule(days) {
  ...
  if (_.isEqual(days, weekday)) return "Mon to Fri (Exclude P.H.)"
  else if (_.isEqual(days, weekend)) return "Sat, Sun (Exclude P.H.)"
  else return _.filter(week, d => days[d]).join(", ")
}
  ...
  const route = await m.Route.build(
    {
      ...
      transportCompanyId: parseInt(process.env.BEELINE_COMPANY_ID),
      ...
      notes: {
        ...
        noPasses: 10,
        crowdstartExpiry,
        tier: [{ pax: 12, price: DEFAULT_CROWDSTART_PRICE }],
      },
      features: `
    ...
    config: {
      ...
      validate: {
        ...
        payload: {
          ...
          matchDaysOfWeek: Joi.boolean(),
          dataSource: Joi.string(),
          imputedDwellTime: Joi.number(),
          includeAnonymous: Joi.boolean(),
          createdSince: Joi.number(),
          suboptimalStopChoiceAllowance: Joi.number(),
        },
      },
