...
import {handleRequestWith, authorizeByRole, assertFound, inSingleDBTransaction} from "../util/endpoints"
...
export function register (server, options, next) {
  ...
  server.route({
    ...
    config: {
      ...
      validate: {
        params: {
          ...
          tag: Joi.string().description('The route tag, or "all" for all route tags'),
        }
      }
    },
    ...
    handler: handleRequestWith(
      ...
      (ignored, request) => {
        ...
        return tag === 'all' ? { userId, companyId, tag: null } : { userId, companyId, tag }
      },
      (replacements, request, {db}) => db.query(`
  ...
  const expireRoutePass = async (routePassInst, {auth, params}, {db, models, transaction}) => {
    ...
    InvalidArgumentError.assert(
      ...
      `Only valid route passes can be expired. This route pass is [${routePassInst.status}]`
    )
    ...
    assert(price, 'Unable to find price that route pass was bought at. This is needed for accounting purposes.')
    const expiryAccount = await models.Account.getByName('Upstream Route Credits', {
      transaction
    })
  ...
  server.route({
    ...
    config: {
      tags: ["api"],
      description: "Expire a number of route passes, choosing the oldest ones first",
      auth: {access: {scope: ['admin', 'superadmin']}},
      validate: {
        params: {
    ...
    handler: handleRequestWith(
      ...
      inSingleDBTransaction(
        (ignored, {payload, params}, {models, transaction}) => models.RoutePass.findAll(
          { where: {...params, status: 'valid'}, limit: payload.quantity, order: 'id', transaction }
        ),
        (routePassInsts, request, context) => Promise.all(
  ...
  server.route({
    method: "POST",
    path: "/companies/{companyId}/route_passes/{tag}/users/{userId}/{routePassId}/expire",
    config: {
      tags: ["api"],
    ...
    handler: handleRequestWith(
      authorizeRouteManager,
      inSingleDBTransaction(
        (ignored, {params}, {models, transaction}) => models.RoutePass.findById(params.routePassId, {transaction}),
        assertFound,
        expireRoutePass
      )
    ),
  ...
  server.route({
    ...
    handler: handleRequestWith(
      ...
      ({companyId, tag, userId}, request, {models}) => models.TransactionItem.findAll({
        where: {
          itemType: 'routePass',
        ...
        include: [
          ...
          { model: models.RoutePass, as: 'routePass', where: {companyId, tag, userId}, attributes: ['id'] }
        ]
      }),
