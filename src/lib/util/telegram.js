...
const options = {
  ...
  reply_markup: {
    ...
    keyboard: [[{ text: "Share contact now", request_contact: true }]]
  }
};

const token = process.env.TELEGRAM_API_TOKEN;
const bot = token ? new TelegramBot(token) : undefined;
      ...
      if (text === "/resend") {
        ...
        updateEventSubsToTelegram(chatId, { phone, email })
          .then(() =>
            bot.sendMessage(
              chatId,
              "Success! You will now receive notifications through Telegram"
            )
          )
        ...
        bot.sendMessage(
          chatId,
          "Verification failed! Please try again, /resend or /quit"
        );
      }
    }
  };
        ...
        : {
            $and: [
              ...
              db.where(
                db.fn(
                  "replace",
                  db.literal("(agent->>'telephone')::text"),
                  " ",
                  ""
                ),
                phone
