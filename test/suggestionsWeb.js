...
lab.experiment("Suggestions from the web", function() {
  ...
  lab.test("Suggestions are limited to owning company", async function() {
    ...
    await admin.addTransportCompany(company.id, {
    ...
    expect(
      JSON.parse(suggestionResponse2.result).find(x => x.id === suggestion1.id)
    ).exist()
    expect(
      JSON.parse(suggestionResponse2.result).find(x => x.id === suggestion2.id)
    ).not.exist()
  })
})
