...
lab.experiment("tickets", function() {
  ...
  */
  ...
  lab.test(
    "query and update tickets with transportCompanyId",
    { timeout: 10000 },
    async function() {
      // pull tickets
      let ticketResponse = await server.inject({
        method: "GET",
        url: "/tickets",
        headers: userToken,
      })
      ...
      const ticketStatusTo = async status =>
        server.inject({
          method: "PUT",
      ...
      let transportCompanyId = companyInstance.id + 1

      let ticket2Response = await server.inject({
        method: "GET",
